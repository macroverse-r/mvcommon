...
mv_title <- function(title, level_adjust = 0, reset = FALSE, .envir = parent.frame()) {
  ...
  # Combine all reset conditions: explicit, case-based, execution context, or execution anchor change
  should_reset <- reset
  ...
  mv_debug(sprintf("Reset decision: explicit=%s, case_based=%s, context_reset=%s, anchor_reset=%s (user_depth=%d), final=%s", 
                   reset, case_info$should_reset, execution_context_reset, execution_anchor_reset, user_depth, should_reset), debug = debug)
