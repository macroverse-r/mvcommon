...
mv_title <- function(title, level_adjust = 0, reset = FALSE, .envir = parent.frame()) {
  ...
  mv_debug(sprintf("mv_title: user_depth=%d, title='%s', counters=[%d,%d,%d]", 
                   ...
                   .mvcommon_env$level1, .mvcommon_env$level2, .mvcommon_env$level3), debug = debug)
  ...
  if (effective_depth <= 3) {
    # Increment counter for given level and reset deeper levels (inlined .increment_level)
    ...
    # Build section number based on current level (inlined .build_section_number)
    if (effective_depth == 1) {
      section_number <- as.character(.mvcommon_env$level1)
    } else if (effective_depth == 2) {
      if (.mvcommon_env$level1 > 0) {
        section_number <- paste0(.mvcommon_env$level1, ".", .mvcommon_env$level2)
      } else {
        section_number <- as.character(.mvcommon_env$level2)
      }
    } else if (effective_depth == 3) {
      if (.mvcommon_env$level1 > 0 && .mvcommon_env$level2 > 0) {
        section_number <- paste0(.mvcommon_env$level1, ".", .mvcommon_env$level2, ".", .mvcommon_env$level3)
      } else if (.mvcommon_env$level2 > 0) {
        section_number <- paste0(.mvcommon_env$level2, ".", .mvcommon_env$level3)
      } else {
        section_number <- as.character(.mvcommon_env$level3)
      }
    }
